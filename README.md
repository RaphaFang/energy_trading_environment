...
2. ~~用 varmelast 的分類產熱驗排程行為~~ → ✅ **2026-08-12 做完**,見 ②(h) 與 `STATUS.md` §8。
   **接下來該做的是它指出的問題**:給 `solve()` 加**最小負載**(先用 `dea.availability()`
   已經備好的資料),再重跑 `validate.py` 看尖峰鍋爐的日內 ρ 有沒有往 +0.170 移動。
   這是現在唯一有明確成功判準的模型改進。
3. **蓄熱槽對齊**:真實 6,500 MWh vs `Plant.s_max` 2,880。
   (`validate.run_model()` 已經改用真值,但 `Plant` 的預設仍是 DEA 單台。)
4. **`eb_max`/`hp_max` 的預設值**:DEA 單台 20/10 對 1,000 MW_th 的熱網明顯太小,
   實測下界是 98/25.9。改預設之前要先決定「`Plant` 代表一台機組還是一個系統」。
5. **`source` 五元組 + `Fuel` 拆出來**(見 ⑧)。
6. 複製成多業者接上多 agent 迴圈。
