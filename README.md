...
| **外部參數**        | ✅ **2026-08-15 `PLACEHOLDERS` 清空**:τ=1.07 / κ=8.50 真值、θ_h 區間 23.8–30.0 已驗證不影響調度、θ_f 排除。見 ⑤b |
...
| **下一步**          | ① `LITERATURE.md`(範圍已重新界定,見 ⑦)② mFRR 資料(**使用者接手中**)③ 多 agent 層 |
...
而且連續推翻了**四個**我自己的推測 —— 都留在 ⑨ 與 `STATUS.md` §8–9。
...
| θ_h 垃圾熱稅 | **23.8**(區間 23.8–30.0) | 2026 公布值 49.4 DKK/GJ_heat;上界用 2024 代理 | ✅ 收掉,見下 |
| θ_f 氣碳稅 | **0**(維持) | 掃描排除 | ✅ 關閉 |

**θ_h 收掉的理由**:門檻(垃圾機組不再全開)在 **35 EUR/MWh_th = 73 DKK/GJ_heat**,
而窗口區間 **49–62 DKK/GJ_heat 完全落在門檻之下** —— 兩端的垃圾供熱佔比 99.8% → 99.5%,
**無實質差異** → **不必再去挖 2021–2024 的真值**。

🔴 **三件必須記住的事**:
① **θ_h 與 κ 是耦合的** —— 擠掉垃圾的是 **power-to-heat 不是尖峰鍋爐**,
所以門檻由 κ 決定(κ=0 時 30,κ=8.50 時 35)。**不能各自判斷。**
② **θ_h 從 0 變 23.8 不是零變更**:ARC/ARGO 供熱成本從 −47.5 移到 **−23.7**,舊基準作廢。
...
## ⑦ 立即可做的下一步(2026-08-15 重排)

### 🔴 1. `LITERATURE.md` —— **範圍已由使用者重新界定,不要照舊的市場力框架寫**

整份仍是電池線的(儲能賽局、Rosen、Monderer-Shapley),CHP/區域供熱一篇都沒有。
**但這一輪的結果動搖了原本的框架本身**,所以範圍改成兩塊:

- **調度模型驗證**(model validation for dispatch models)
- **P2H 參與備轉/調頻市場**

**為什麼不能照舊寫**(使用者的論證,記在這裡免得重來):
① 市場力分析的做法是「建完全競爭基準 → 實測與基準的偏離 = 策略行為」,
但 ②(h) 測出來**這個基準的時點是錯的** → 殘差裡混著策略行為與基準誤差,
而後者量級不小(熱電 Qc 的 ΔR² 差 48×)。**時點修好前,任何策略行為的量化都會把模型誤差算成市場力。**
② 垃圾焚化(27.7%)成本強負值、永遠全開、**沒有策略行為** → 直接出局;
剩下能談市場力的只有熱電那 **64.6%**,而那正是被生質價擋住的三台。
→ 若備轉收入真的主導 P2H 調度,它在成本式裡是一個**漏掉的收益項,不是新框架**。
**這個判斷做完,才知道市場力那條線值不值得投。**

### 2. mFRR / 調頻資料 —— **使用者接手中**

實測電鍋爐日總量跟著**熱需求**走(ρ=+0.335),模型跟著**價格**走(ρ=−0.74)。
這是目前最大的未解,而且候選解釋只剩這一個(κ 已被排除:nettarif 是能源費、不隨小時變動)。
已確認存在且 schema 對得上(各回 HTTP 200):`RegulatingBalancePowerdata`
(逐時、分價區、mFRR 實際啟動量)、`FcrReservesDK2`、`MfrrReservesDK2`。
⚠️ `AfrrReservesDK2` **是 404,別照命名規律推**。批量抓取撞 Energinet 429。

### 3. 生質燃料價(使用者處理)—— 範圍比先前寫的窄

✅ **2025–2026 有**(`assumptions.biomass_fuel_price_eur_mwh()`,SØB25 Tabel 2,
木片 €36.3/MWh_fuel);🔴 **2021–2024 完全沒有**,且 2025 那格只是遠期觀點。
⚠️ **它擋住的是水準比對,不是時點結論**(2026-08-13 掃描證實)。
解掉之後 `dk2_fleet.runnable()` 從 2/6 變 5/6。

### 4. 其餘(沒有先後,都不擋)

- **蓄熱槽對齊**:真實 6,500 MWh vs `Plant.s_max` 2,880(`validate.run_model()` 已用真值)
- **`eb_max`/`hp_max` 預設值**:DEA 單台 20/10 vs DK2 實測下界 98/25.9。
  改之前要先決定「`Plant` 代表一台機組還是一個系統」
- **對稱性缺口**:CHP 側的 indfødningstarif + balancetarif(≈ €1.5/MWh_e)該打在 `P⁺` 上,
  目前式子裡沒有。十分鐘的事,但會動到賣電收益
- **`source` 五元組 + `Fuel` 拆出來**(見 ⑧)
- 複製成多業者接上**多 agent 層**(核心假說目前仍無法檢驗)

### ❌ 已經關閉、不要再做的

- **爬坡約束** —— 每小時 2.4–9.0 倍滿載,逐時 LP 加了是白做的
- **查天然氣國內碳稅 θ_f** —— 掃描證明它不修時點,還讓尖峰鍋爐離實測更遠
- **挖 2021–2024 的垃圾熱稅真值** —— 公布區間全在門檻之下,調度無實質差異
...
- **不要引用舊的 ARC/ARGO 供熱成本數字。** 這個數字被改過三次:
  ...
  → **現在 −23.7**(θ_h 填真值 23.8)。引用前先確認是哪一組、θ_h 是多少。
