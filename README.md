...
| **「昨天同一小時的實測值」多半是 leak** | 日前 **D−1 12:00 關門**,而「昨天同一小時」對下午的目標指的是關門**之後**才量到的。`load_lag24`/`residual_lag24` 因此不合法(已進 `LEAK_COLS`),但 **`price_lag24` 合法** —— 日前價 D−2 就公布了。合法替代 = 再往前推一天(`*_lag48`) |
