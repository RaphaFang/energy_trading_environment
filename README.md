...
| **排程行為**        | 🔴 **2026-08-12 首次驗證:年佔比對得上,日內時點是反的**。LP 對價格系統性過度反應,見 ②    |
...
### (h) 🔴 排程行為驗證:**年佔比對得上,日內時點是反的**(2026-08-12,`heat/validate.py`)
...
| 電鍋爐 日總 ρ(價)   | −0.254     | **−0.785** | ← 過度反應 3×    |
| 電鍋爐 日總 ρ(需求) | **+0.418** | **−0.099** | ← 實測靠需求決定 |
| 熱電 Qc 的 ΔR²      | 0.004      | **0.250**  | ← **60×**        |

🔴 **只看年佔比會誤判成「模型驗證通過」** —— 這是「驗證要挑有識別力的標的」的第二個實例。
**LP 在每一個可測維度上都對價格過度反應** → 指向缺**機組啟停/最小負載**約束
(`chp.py` 第一版刻意不做),**不是**燃料價或稅費的問題:加 κ=25.3 只改水準
(P2H 4.13%→2.43%),行為簽名完全不動。已用抽汽原型控制變數試過,
`cv` 把尖峰鍋爐 ρ 往正推(cv=0 → −0.345、cv=0.15 → −0.198)但**翻不到 +0.170**。
...
| **不要用行為反推稅費**             | 電鍋爐開關反推的 τ+κ 在不同樣本間 **+€25.5 ↔ −€16.5**。剛好貼近 soeB25 的 €25.3 是巧合,不是證據 |
